...
class Tree extends event_emitter_1.LargeDomEventEmitter {
  ...
  render(nodes) {
    ...
    if (this.mainHolderHtmlNode.classList && !this.mainHolderHtmlNode.classList.contains(TreeConstants_js_1.TreeConstants.TreeCssClassNames.MAIN_CLASS_NAME)) {
    ...
    const {
      itemsAmount,
      objectKeys
    } = ArrayOrObjectPackage_js_1.ArrayOrObjectPackage.getArrayOrObjectItemsAmount(isArray, this.data);
  ...
  checkDataNodeSubtree(node) {
    ...
    const {
      dataTypeString,
      dataType
    } = ArrayOrObjectPackage_js_1.ArrayOrObjectPackage.getDataTypeStringAndConst(subtreeJsonNodes);
    const isArray = dataType === ArrayOrObjectPackage_js_1.ArrayOrObjectPackage.JsonDataType.ARRAY ? 1 : 0;
    if (!subtreeJsonNodes || subtreeJsonNodes.length === 0 || dataType !== ArrayOrObjectPackage_js_1.ArrayOrObjectPackage.JsonDataType.ARRAY && dataType !== ArrayOrObjectPackage_js_1.ArrayOrObjectPackage.JsonDataType.OBJECT) {
      return {
        isArray,
    ...
    const {
      itemsAmount,
      objectKeys
    } = ArrayOrObjectPackage_js_1.ArrayOrObjectPackage.getArrayOrObjectItemsAmount(isArray, subtreeJsonNodes);
  ...
  renderOneTreeNode(node, nodePosition, nodeKey, flatNodeHolderClone, holder) {
    ...
    const nodeHtml = this.templateRenderer.setTemplate(TreeConstants_js_1.TreeConstants.TEMPLATE__TREE_HTML_NODE)
    // @ts-ignore
    .setData(dataForRendering).render();
    ...
    } else if (!node[this.metadata.NODE__OPENED] && this.nodesOpenedMode === TreeConstants_js_1.TreeConstants.NodesOpenedMode.JSON_DATA_DEFINED) {
      ul.style.display = "none";
  ...
  updateDataNodeIdAndPath(node, nodePosition, nodeKey, flatNodeHolderClone, holder) {
    ...
    const pathInJsonArray = [...pathInJsonOfNodeHolder, pathKeyInNodeHolder];
  ...
  escapeHTMLForAttribute(str) {
    return str.replace(/"/g, "&quot;") // Replace double quotes

...
if [ $is_www_dockerized = 1 ]; then
  ...
  docker compose exec "${npm_www_dockercompose_service_name}" /usr/bin/env bash -c 'npm --prefix "${npm_index_catalog}/build_tools" run watch'
else
  cd "${npm_index_catalog}"
  npm --prefix ./build_tools run watch
fi
